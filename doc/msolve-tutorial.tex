...
\item The flag \verb+-v <int>+ or \verb+--verbose <int>+ controls the verbosity

  \hfill \verb+Default value: 0+

\item The flag \verb+-t <int>+ or \verb+--threads <int>+
  controls the number of threads used

  \hfill \verb+Default value: 1+

\item The flag \verb+-p <int>+ or \verb+--precision <int>+ controls the binary
  precision of the output of the
...
\item The flag \verb+-g <int>+ or \verb+--groebner-basis+ tells \msolve to output
...
\item The flag \verb+-P <int>+ or \verb+--precision <int>+ tells \msolve to output the
...
\item The flag \verb+-c <int>+ tells \msolve how to handle genericity
  ...
  linear form until the genericity requirements are satisfied. In
  positive characteristic, this linear have random coefficients, while
  in characteristic $0$, it is deterministically chosen, depending on
  the input.
...
\item The flag \verb+--random-seed <int>+ tells \msolve which seed must be used
  to initialize the pseudo-random generator: \verb+0+ means that
  \verb+time(0)+ is used so that the seed is based on current time,
  otherwise, for any integer \verb+N+, \verb+N+ will be the seed. The
