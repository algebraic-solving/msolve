...
void mpz_poly_print_maple_file(FILE *file, mpz_t *pol, unsigned long int deg){
  fprintf(file, "pol:=");
  ...
  fprintf(file, "st:=time():\nsols:=RootFinding[Isolate]([pol],[x]):time()-st;\nlprint(nops(sols));lprint(sols);");
}
...
int main(int argc, char **argv){
  ...
  mpz_poly_print_maple_file(file_out, upoly, deg);
