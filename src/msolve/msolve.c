...
static inline int new_rational_reconstruction(
    ...
    mpq_matfglm_t mpq_mat, crt_mpz_matfglm_t crt_mat, 
    nvars_t nlins, nvars_t *linvars, uint32_t *lineqs, mpz_t *crt_linear_forms,
    mpz_t *mpq_linear_forms, mpz_t *mpz_linear_forms,
    ...
    int *is_lifted, int *mat_lifted, int *lin_lifted, int doit, int nthrds,
  ...
  *matrec = *matrec_checked;
  ...
  if (nlins && (*lin_lifted) < 2) {
    for (int i = 0; i < nlins; i++) {
      crt_lift_dense_rows(crt_linear_forms + i * (nmod_param->nvars + 1),
                          lineqs + i * (nmod_param->nvars + 1), 0,
                          nmod_param->nvars + 1, modulus, prod_crt, prime,
                          trace_det->tmp, nthrds);
    }
  }
  if (*matrec_checked < crt_mat->nrows) {
    crt_lift_mat(crt_mat, mat, modulus, prod_crt, prime, trace_det->tmp,
                 *matrec_checked, nthrds);
  }
#endif
...
/*
  ...
  */
  ...
  if (nlins && (*lin_lifted) < 2) {
    ...
    for (int i = 0; i < nlins; i++) {
      boo =
          rat_recon_array(mpq_linear_forms + (2 * i * (nmod_param->nvars + 1)),
                          crt_linear_forms + (i * (nmod_param->nvars + 1)),
                          nmod_param->nvars + 1, modulus, recdata);
      if (boo == 0)
        break;
    }
    if (boo) {
      build_linear_forms(mpz_linear_forms, mpq_linear_forms,
                         nmod_param->nvars + 1, nlins);
      *lin_lifted = 1;
    }
  }
  ...
  if (*matrec_checked < crt_mat->nrows) {
    long old = *matrec;
    rat_recon_matfglm(mpq_mat, crt_mat, modulus, recdata, rnum, rden,
                      guessed_den, matrec, mat_lifted);
    if((*matrec) > old){
        *mat_lifted = 1;
    }
  }

  if ((*matrec_checked)>1 && (*matrec_checked) <= mpq_mat->nrows ) {
     /* 
    for (uint32_t i = MAX(0, (*oldmatrec_checked) - 1); i < (*matrec_checked); i++) {
      mpz_lcm(*guessed_den, *guessed_den, mpq_mat->denoms[i]);
    }
    */
//    mpz_set(*guessed_den, mpq_mat->denoms[(*matrec_checked) - 1]);
    mpz_gcd(*guessed_den, mpq_mat->denoms[(*matrec_checked) - 1], mpq_mat->denoms[(*matrec_checked) - 2]);
    mpz_divexact(*guessed_den, mpq_mat->denoms[(*matrec_checked) - 1], *guessed_den);
...
static int32_t *
initial_modular_step(sp_matfglm_t **bmatrix, int32_t **bdiv_xn,
                     int32_t **blen_gb_xn, int32_t **bstart_cf_gb_xn,
                     ...
                     int32_t *num_gb, int32_t **leadmons, uint64_t *bsz,
                     param_t **bparam, bs_t *gbg, md_t *md, const int32_t fc,
                     int print_gb, int *dim, long *dquot_ori,
                     data_gens_ff_t *gens, files_gb *files, int *success) {
  double rt = realtime();
  ...
  check_and_set_linear_poly(nlins_ptr, linvars, lineqs_ptr, bs->ht, leadmons[0],
                            bs);
  if (has_dimension_zero(bs->lml, bs->ht->nv, leadmons[0])) {
    ...
    if (print_gb == 0) {
      *bmatrix = build_matrixn_from_bs_trace(
          bdiv_xn, blen_gb_xn, bstart_cf_gb_xn, lmb, dquot, bs, bs->ht,
          leadmons[0], bs->ht->nv, fc, md->info_level);
      if (*bmatrix == NULL) {
        *success = 0;
        *dim = 0;
        *dquot_ori = dquot;
      ...
      *bsz = bs->ht->nv - (*nlins_ptr); // nlins ;
      ...
      check_and_set_vars_squared_in_monomial_basis(squvars, lmb, dquot,
                                                   gens->nvars);
      *bparam = nmod_fglm_compute_trace_data(
          *bmatrix, fc, bs->ht->nv, *bsz, *nlins_ptr, linvars, lineqs_ptr[0],
          squvars, md->info_level, bdata_fglm, bdata_bms, success, md);
    }
...
static void secondary_modular_steps(
    sp_matfglm_t **bmatrix, int32_t **div_xn, int32_t **len_gb_xn,
    int32_t **start_cf_gb_xn,

    nvars_t *bnlins, nvars_t **blinvars, uint32_t **blineqs, nvars_t **bsquvars,

    fglm_data_t **bdata_fglm, fglm_bms_data_t **bdata_bms,

    int32_t *num_gb, int32_t **leadmons_ori, int32_t **leadmons_current,

    uint64_t bsz, param_t **nmod_params,
    /* trace_t **btrace, */
    bs_t *bs_qq, md_t *st, const int32_t fc, int info_level, bs_t **bs,
    int32_t *lmb_ori, int32_t dquot_ori, primes_t *lp, data_gens_ff_t *gens,
    double *stf4, const long nbsols, uint32_t *bad_primes,
    mpq_matfglm_t mpq_mat, int *mat_lifted, mpz_t *mpz_linear_forms,
    int *lin_lifted, deg_t *oldmatrec_checked, deg_t *matrec_checked) {
  st->info_level = 0;
  st->f4_qq_round = 2;
...
#pragma omp parallel for num_threads(nthrds) private(i) schedule(static)
  for (i = 0; i < st->nprimes; ++i) {
    if (*mat_lifted < 2 || *lin_lifted < 2) {
    ...
    if (*mat_lifted == 2 && *lin_lifted == 2) {
      compute_modular_linear_forms(bnlins[i], bs_qq->ht->nv + 1, blineqs[i],
                                   mpz_linear_forms, lp->p[i]);
      compute_modular_matrix(bmatrix[i], mpq_mat, lp->p[i]);
    } else {
      if (equal_staircase(leadmons_current[i], leadmons_ori[i], num_gb[i],
        ...
        build_matrixn_from_bs_trace_application(
            bmatrix[i], div_xn[i], len_gb_xn[i], start_cf_gb_xn[i], lmb_ori,
            dquot_ori, bs[i], bs[i]->ht, leadmons_ori[i], bs[i]->ht->nv,
            lp->p[i]);
      } else {
        bad_primes[i] = 1;
      }
...
static inline int is_lucky_matmul_prime_ui(uint32_t prime,
  ...
  for (uint32_t i = 0; i < mpq_mat->nrows; i++) {
    if (mpz_fdiv_ui(mpq_mat->denoms[i], prime) == 0) {
      return 1;
    }
  }
  return 0;
...
int msolve_trace_qq(mpz_param_t *mpz_paramp, param_t **nmod_param, int *dim_ptr,
                    long *dquot_ptr, data_gens_ff_t *gens,
                    int32_t ht_size, // initial_hts,
                    int32_t nr_threads, int32_t max_nr_pairs,
                    int32_t elim_block_len, int32_t reset_ht, int32_t la_option,
                    int32_t use_signatures, int32_t info_level,
                    int32_t print_gb, int32_t pbm_file, files_gb *files,
                    int round) {
  ...
  int32_t *lmb_ori =
      initial_modular_step(bmatrix, bdiv_xn, blen_gb_xn, bstart_cf_gb_xn,
                           ...
                           &nlins, blinvars[0], lineqs_ptr, squvars,

                           bdata_fglm, bdata_bms,

                           num_gb, leadmons_ori,

                           &bsz, nmod_params, bs_qq, st,
                           lp->p[0], // prime,
                           print_gb, dim_ptr, dquot_ptr, gens, files, &success);
  ...
  if (nlins) {
    ...
    mpz_linear_forms = mpz_linear_forms_allocate(nlins, nv);
  }
#endif
  ...
  if (lmb_ori == NULL || success == 0 || print_gb || gens->field_char) {
    ...
    crt_linear_forms_clear(crt_linear_forms, nlins, nv);
    mpq_linear_forms_clear(mpq_linear_forms, nlins, nv);
#endif
    if (print_gb) {
      return 0;
    }
  ...
  duplicate_data_mthread_trace(
      st->nthrds, bs_qq, st, num_gb, leadmons_ori, leadmons_current, bdata_bms,
      bdata_fglm, bstart_cf_gb_xn, blen_gb_xn, bdiv_xn, bmatrix, nmod_params,
      nlins, bnlins, blinvars, lineqs_ptr, bsquvars);
  ...
  while (rerun == 1 || mcheck == 1) {
    ...
    double stf4 = 0;
    secondary_modular_steps(
        bmatrix, bdiv_xn, blen_gb_xn, bstart_cf_gb_xn,

        bnlins, blinvars, lineqs_ptr, bsquvars,

        bdata_fglm, bdata_bms, num_gb, leadmons_ori, leadmons_current,

        bsz, nmod_params,         /* btrace, */
        bs_qq, st, field_char, 0, /* info_level, */
        bs, lmb_ori, *dquot_ptr, lp, gens, &stf4, nsols, bad_primes, mpq_mat,
        &mat_lifted, mpz_linear_forms, &lin_lifted, &oldmatrec_checked, &matrec_checked);
      ...
      if (bad_primes[i] == 0) {
        ...
        if (mcheck == 1) {
          br = new_rational_reconstruction(
              *mpz_paramp, tmp_mpz_param, nmod_params[i], mpq_mat, crt_mat,
              bnlins[i], blinvars[i], lineqs_ptr[i], crt_linear_forms,
              mpq_linear_forms, mpz_linear_forms, trace_det, bmatrix[i], numer,
              ...
              &mat_lifted, &lin_lifted, doit, st->nthrds, info_level);
...
int real_msolve_qq(mpz_param_t *mpz_paramp, param_t **nmod_param, int *dim_ptr,
                   ...
                   int32_t nr_threads, int32_t max_nr_pairs,
                   int32_t elim_block_len, int32_t reset_ht, int32_t la_option,
                   int32_t use_signatures, int32_t info_level, int32_t print_gb,
                   int32_t pbm_file, int32_t precision, files_gb *files,
                   int round, int32_t get_param) {
  ...
  */
  ...
  double ct0 = cputime();
  double rt0 = realtime();
  int b = msolve_trace_qq(mpz_paramp, nmod_param, dim_ptr, dquot_ptr, gens,
                          ht_size, // initial_hts,
                          nr_threads, max_nr_pairs, elim_block_len, reset_ht,
                          la_option, use_signatures, info_level, print_gb,
                          pbm_file, files, round);
  double ct1 = cputime();
  double rt1 = realtime();
...
int core_msolve(int32_t la_option, int32_t use_signatures, int32_t nr_threads,
                int32_t info_level, int32_t initial_hts, int32_t max_pairs,
                int32_t elim_block_len, int32_t update_ht, int32_t generate_pbm,
                int32_t reduce_gb, int32_t print_gb, int32_t truncate_lifting,
                int32_t get_param, int32_t genericity_handling,
                int32_t saturate, int32_t colon, int32_t normal_form,
                int32_t normal_form_matrix, int32_t is_gb, int32_t precision,
                files_gb *files, data_gens_ff_t *gens, param_t **paramp,
                mpz_param_t *mpz_paramp, long *nb_real_roots_ptr,
                interval **real_roots_ptr, real_point_t **real_pts_ptr) {
...
restart:
  ...
  if (gens->field_char > 0) {
    if (use_signatures > 0) {
      ...
      success = initialize_gba_input_data(
          &bs, &bht, &st, gens->lens, gens->exps, (void *)gens->cfs, 1073741827,
          0 /* DRL order */, elim_block_len, gens->nvars,
          /* gens->field_char, 0 [> DRL order <], gens->nvars, */
          gens->ngens, saturate, initial_hts, nr_threads, max_pairs, update_ht,
          la_option, use_signatures, 1 /* reduce_gb */, 0, info_level);

      if (st->homogeneous != 1) {
        fprintf(stderr, "Input system must be homogeneous.\n");
        exit(1);
      }

      st->gfc = gens->field_char;
      if (info_level) {
        fprintf(stderr, "NOTE: Field characteristic is now corrected to %u\n",
                st->gfc);
      }
      if (!success) {
      ...
      int64_t nb = export_results_from_gba(bld, blen, bexp, bcf, &malloc, &bs,
                                           &bht, &st);
    ...
    if (saturate == 1) { /* positive characteristic */
      ...
      /*             initialize generators of ideal, note the
       * "gens->ngens-normal_form" which means that we only take the first
       * nr_gens-normal_form generators from the input file, the last
       * normal_form polynomial in the file will be reduced w.r.t. the basis
       *
       *             NOTE: There is a little hack here, instead of
       * gens->field_char we give 1073741827 as parameter, which ensures that
       * all F4 internal routines are the 32-bit implementations (since nf is at
       * the moment only implemented for 32-bit elements). Later on we set st-fc
       * by hand to the correct field characteristic. */
      success = initialize_gba_input_data(
          &bs, &bht, &st, gens->lens, gens->exps, (void *)gens->cfs, 1073741827,
          0 /* DRL order */, elim_block_len, gens->nvars,
          /* gens->field_char, 0 [> DRL order <], gens->nvars, */
      ...
      if (st->ff_bits < 32) {
        fprintf(stderr, "Error: not implemented yet (prime field of too low "
                        "characteristic)\n");
        return 1;
      }
      if (!success) {
      ...
      if (is_gb == 1) {
        ...
        sat->ht = bht;
        import_input_data(sat, st, gens->ngens - saturate, gens->ngens,
        ...
        if (print_gb) {
          print_ff_basis_data(files->out_file, "a", bs, bht, st, gens,
                              print_gb);
        }
      }
      return 0;
    ...
    if (colon == 1) {
      ...
      int success = initialize_gba_input_data(
          &bs, &bht, &st, gens->lens, gens->exps, (void *)gens->cfs, 1073741827,
          0 /* DRL order */, elim_block_len, gens->nvars,
          /* gens->field_char, 0 [> DRL order <], gens->nvars, */
          gens->ngens, 1, initial_hts, nr_threads, max_pairs, update_ht,
          la_option, use_signatures, 1 /* reduce_gb */, 0, info_level);
...
----------------------------------------\n");
      ...
      tbr->ht = bht;
      import_input_data(tbr, st, gens->ngens - 1, gens->ngens, gens->lens,
                        gens->exps, (void *)gens->cfs, NULL);
      tbr->ld = tbr->lml = 1;
      /* normalize_initial_basis(tbr, st->gfc); */
      for (int k = 0; k < 1; ++k) {
...
----------------------------------------\n");
      ...
      /* size of the list */
      long suppsize =
          tbr->hm[tbr->lmps[1]][LENGTH]; // bs->hm[bs->lmps[1]][LENGTH]
      ...
      get_normal_form_matrix(tbr, bht, 1, st, &sht, &hcm, &mat);
...
#define ZERO 0
#if ZERO
      int32_t *lmb =
          monomial_basis_colon(bld[0], gens->nvars, bexp_lm, &dquot, maxdeg);
#else
      int32_t *lmb = monomial_basis_colon_no_zero(bld[0], gens->nvars, bexp_lm,
                                                  &dquot, maxdeg);
#endif
      /* printf("\nMonomial basis:\n"); */
      ...
      uint32_t **leftvectorsparam =
          malloc(2 * (gens->nvars - 1) * sizeof(uint32_t *));
      ...
      /* we need the nf of sigma x_n for all sigma is this
         support */
#if ZERO
      sp_matfglmcol_t *matrix = build_matrixn_colon(
          lmb, dquot, bld[0], blen, bexp, bcf_ff, bexp_lm, tbr, bht, st, mul,
          bs, gens->nvars, gens->field_char, maxdeg, gens, leftvector,
          leftvectorsparam, suppsize);
#else
      sp_matfglmcol_t *matrix = build_matrixn_colon_no_zero(
          lmb, dquot, bld[0], blen, bexp, bcf_ff, bexp_lm, tbr, bht, st, mul,
          bs, gens->nvars, gens->field_char, maxdeg, gens, leftvector,
          leftvectorsparam, suppsize);
#endif
...
----------------------------------------\n");
      ...
      param_t *param = nmod_fglm_guess_colon(
          matrix, gens->field_char, leftvector, leftvectorsparam, gens->nvars,
...
----------------------------------------\n");
        fprintf(stderr, "INIT   PERCTG %13.2f%% (REAL) / %5.2f%% (CPU)\n",
                100 * (rt0p - rt0) / (rt4 - rt0),
                100 * (ct0p - ct0) / (ct4 - ct0));
        fprintf(stderr, "F4     PERCTG %13.2f%% (REAL) / %5.2f%% (CPU)\n",
                100 * (rt1 - rt0p) / (rt4 - rt0),
                100 * (ct1 - ct0p) / (ct4 - ct0));
...
----------------------------------------\n");
    ...
    if (normal_form == 0) { /* positive characteristic */
      ...
      if (elim_block_len > 0 && print_gb == 0) {
        if (info_level) {
          fprintf(stderr, "Warning: elim order not available for rational "
                          "parametrizations\n");
          fprintf(stderr, "Computing Groebner basis\n");
          print_gb = 2;
        }
      }
      b = real_msolve_qq(mpz_paramp, &param, &dim, &dquot, nb_real_roots_ptr,
                         real_roots_ptr, real_pts_ptr, gens, initial_hts,
                         nr_threads, max_pairs, elim_block_len, update_ht,
                         la_option, use_signatures, info_level, print_gb,
      ...
      manage_output(b, dim, dquot, files, gens, param, mpz_paramp, get_param,
                    nb_real_roots_ptr, real_roots_ptr, real_pts_ptr,
                    info_level);
      ...
      if (b == 1) {
        ...
        fprintf(stderr,
                "be done automatically if you run msolve with option\n");
      ...
      /* initialize generators of ideal, note the "gens->ngens-normal_form"
       * which means that we only take the first nr_gens-normal_form generators
       * from the input file, the last normal_form polynomial in the file will
       * be reduced w.r.t. the basis
       *
       * NOTE: There is a little hack here, instead of gens->field_char we
       * give 1073741827 as parameter, which ensures that all F4 internal
       ...
       * to the correct field characteristic. */
      success = initialize_gba_input_data(
          &bs, &bht, &st, gens->lens, gens->exps, (void *)gens->cfs,
      ...
      print_ff_nf_data(files->out_file, "a", 0, normal_form, tbr, bht, st, gens,
                       2);
      if (normal_form_matrix > 0) {
        ...
        get_normal_form_matrix(tbr, tbr->ht, normal_form, st, &sht, &hcm, &mat);
        ...
        /* sparse represented matrix of normal forms, note that the column
         * entries of the rows are not sorted, but you can do so using any sort
         * algorithm */
        printf(
        ...
        for (len_t k = 0; k < mat->nr; ++k) {
          ...
          for (len_t l = 0; l < mat->tr[k][LENGTH]; ++l) {
            printf("%u at %u, ", tbr->cf_32[mat->tr[k][COEFFS]][l],
                   mat->tr[k][l + OFFSET]);
          }
          printf("\n");
        ...
        cf32_t *dr = (cf32_t *)malloc((unsigned long)mat->nc * sizeof(cf32_t));
        for (len_t k = 0; k < mat->nr; ++k) {
          memset(dr, 0, (unsigned long)mat->nc * sizeof(cf32_t));
          printf("row %u | ", k);
          for (len_t l = 0; l < mat->tr[k][LENGTH]; ++l) {
            dr[mat->tr[k][l + OFFSET]] = tbr->cf_32[mat->tr[k][COEFFS]][l];
          }
    ...
    if (saturate == 1) { /* characteristic is 0 and elim_block = 0 */
      ...
      int32_t reduce_gb = 1;
      int res = validate_input_data(
          &invalid_gens, gens->mpz_cfs, gens->lens, &field_char,
          &monomial_order, &elim_block_len, &gens->nvars, &gens->ngens,
          &saturate, &initial_hts, &nr_threads, &max_pairs, &update_ht,
       ...
       * some of the input data is corrupted. */
      if (check_and_set_meta_data_trace(
              st, gens->lens, gens->exps, (void *)gens->mpz_cfs, invalid_gens,
              field_char, 0, elim_block_len, gens->nvars, gens->ngens, saturate,
              initial_hts, nr_threads, max_pairs, update_ht, la_option,
              use_signatures, 1, prime_start, nr_primes, 0, info_level)) {
        free(st);
        return -3;
      }
      ...
      sat_qq->ht = bht;
      import_input_data(sat_qq, st, gens->ngens - saturate, gens->ngens,
      ...
      f4sat_trace_learning_phase_1(trace, tht, bs_qq, sat_qq, &bht, st,
                                   lp->p[0]);
      ...
      f4sat_trace_learning_phase_2(trace, tht, bs_qq, sat_qq, &bht, st,
                                   lp->p[0]);
      ...
      for (i = 0; i < st->nprimes; ++i) {
        ca0 = realtime();
        bs[i] = f4sat_trace_application_phase(trace, tht, bs_qq, sat_qq, lht,
                                              st, lp->p[i]);
...
void msolve_julia(void *(*mallocp)(size_t), int32_t *rp_ld, int32_t *rp_nr_vars,
                  int32_t *rp_dim, int32_t *rp_dquot, int32_t **rp_lens,
                  char ***rp_var_namesp, void **rp_cfs_linear_form,
                  void **rp_cfs, int32_t *n_real_sols, void **real_sols_num,
                  int32_t **real_sols_den, int32_t *lens, int32_t *exps,
                  void *cfs, char **var_names, char *output_file,
                  const uint32_t field_char, const int32_t mon_order,
                  const int32_t elim_block_len, const int32_t nr_vars,
                  const int32_t nr_gens, const int32_t initial_hts,
                  const int32_t nr_threads, const int32_t max_nr_pairs,
                  const int32_t reset_ht, const int32_t la_option,
                  const int32_t use_signatures, const int32_t print_gb,
                  const int32_t get_param, const int32_t genericity_handling,
                  const int32_t precision, const int32_t info_level) {
  /* timinigs */
  double st0 = cputime();
  double rt0 = realtime();
  ...
  for (i = 0; i < nr_vars; ++i) {
    ...
    memcpy(gens->vnames[i], var_names[i],
           (unsigned long)strlen(var_names[i]) * sizeof(char));
  }
  /* gens->lens                  = lens; */
  ...
  int ret = core_msolve(
      la_option, use_signatures, nr_threads, info_level, initial_hts,
      max_nr_pairs, elim_block_len, reset_ht, 0 /* generate pbm */,
      1 /* reduce_gb */, print_gb, 0 /*truncate_lifting*/, get_param,
      genericity_handling, 0 /* saturate */, 0 /* colon */, 0 /* normal_form */,
      0 /* normal_form_matrix */, 0 /* is_gb */, precision, files, gens, &param,
  ...
  *rp_dim = mpz_param->dim;
  ...
  if (mpz_param->dim != -1) {
    export_julia_rational_parametrization_qq(
        mallocp, rp_ld, rp_nr_vars, rp_dim, rp_dquot, rp_lens, &rp_var_names,
        rp_cfs_linear_form, rp_cfs, real_sols_num, real_sols_den, gens,
...
-----------------------------------\n");
    fprintf(stderr,
            "msolve overall time  %13.2f sec (elapsed) / %5.2f sec (cpu)\n",
            rt1 - rt0, st1 - st0);
    fprintf(stderr, "-------------------------------------------------\
-----------------------------------\n");
  }
