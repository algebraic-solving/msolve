...
static inline void display_help(char *str){
  ...
  fprintf(stdout, "\t an encoding by means of rational parametrizations.\n");
  fprintf(stdout, "\n\t When the number of solutions is finite, msolve outputs\n");
  fprintf(stdout, "\t an encoding by means of rational parametrizations.\n");

  fprintf(stdout, "\t This involves univariate polynomials which are encoded by\n");
  fprintf(stdout, "\t a list [deg, LC] where deg is the degree and LC the list of \n");
  fprintf(stdout, "\t its coefficients (by degree ascending order). \n\n");

  fprintf(stdout, "\t - positive characteristic case (less than 2^31): \n");

  fprintf(stdout, "\t\t * Case 1: infinitely many solutions\n");
  fprintf(stdout, "\t\t   [FILE2] contains the minimal Grobner basis of the\n");
  fprintf(stdout, "\t\t   polynomials in [FILE1] for the grevlex ordering\n");
  fprintf(stdout, "\t\t   (see below for the format of [FILE2]);\n");

  fprintf(stdout, "\t\t * Case 2: finitely many solutions\n");
  fprintf(stdout, "\t\t   [FILE2] contains the solutions of the input system given\n");
  fprintf(stdout, "\t\t   by the chosen precision (see flag -p). If the flag -P is set\n");
  fprintf(stdout, "\t\t   to 1, the rational parametrization is given, too:\n");
  fprintf(stdout, "\t\t   When the last coordinate satisfies generic geometric \n");
  fprintf(stdout, "\t\t   properties, [FILE2] contains an encoding of the set \n");
  fprintf(stdout, "\t\t   of solutions by means of a rational parametrization\n");
  fprintf(stdout, "\t\t   (see below for the encoding of such parametrizations)\n");
  fprintf(stdout, "\t\t   The output is encoded as follows: \n");
  fprintf(stdout, "\t\t   [d, n, deg, varstr, linform, elim, den, param]\n");
  fprintf(stdout, "\t\t   where d is the dimension, n is the number of variables\n");
  fprintf(stdout, "\t\t   deg is the degree of the ideal generated by the input\n");
  fprintf(stdout, "\t\t   varstr is an array of the used variables (note that msolve\n");
  fprintf(stdout, "\t\t   might change the order of variables and/or add a new variable\n");
  fprintf(stdout, "\t\t   for applying a linear form in order to achieve genericity)\n");
  fprintf(stdout, "\t\t   linform is an array of coefficients which defines a linear form\n");
  fprintf(stdout, "\t\t   ordered by the order given in varstr (if empty no linear form\n");
  fprintf(stdout, "\t\t   was added)\n");
  fprintf(stdout, "\t\t   The following data delivers now information w.r.t. the variables\n");
  fprintf(stdout, "\t\t   and their order given in varstr:\n");
  fprintf(stdout, "\t\t   elim and den are univariate polynomials, \n");
  fprintf(stdout, "\t\t   param is a list [[p[1], c[1]], ..., [p[n-1], c[n-1]]] \n");
  ...
  fprintf(stdout, "\t\t * Case 1: infinitely many solutions\n");
  fprintf(stdout, "\t\t   [NOT DONE YET] -> we will output the staircase of the GB\n");
