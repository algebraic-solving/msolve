...
static inline void avx2_matrix_vector_product(uint32_t* vec_res,
                                              const uint32_t* mat,
                                              const uint32_t* vec,
                                              const uint32_t *dst,
...
static inline void old_8mul_matrix_vector_product(uint32_t* vec_res,
                                               const uint32_t* mat,
                                               const uint32_t* vec,
                                               const uint32_t *dst,
                                               const uint32_t ncols,
  ...
  __m256i res1;
    ...
    //mask pour recuperer les parties basses
    mask=AVX2SET1_64(MONE32);
    const uint64_t quo0 = LENGTHQ8(ncols);
    const uint64_t rem0 = LENGTHR8(ncols);
    ...
    for(j = 0; j < nrows; ++j){
      ...
      *vec_res=acc64%PRIME;
      ...
      for(long k = 0; k < rem; k++){
        tmp += ( ((uint64_t)mat[k] * (uint64_t)vec_cp[k]) % PRIME );
      }
      *vec_res += ( tmp % PRIME );
      mat+=(rem + dst[j]);
      ++vec_res;
