    ...
    )
{
    ...
    if (ht->ev == NULL) {
        ...
        fprintf(stderr, "esz = %llu, segmentation fault will follow.\n", ht->esz);
    }
    ...
    if (tmp == NULL) {
        ...
        fprintf(stderr, "initialization failed, esz = %llu,\n", ht->esz);
    ...
    )
{
    ...
    if (ht->ev == NULL) {
        fprintf(stderr, "Enlarging hash table failed for esz = %llu,\n", esz);
        fprintf(stderr, "segmentation fault will follow.\n");
    }
    ...
    if (ht->ev[0] == NULL) {
        ...
        fprintf(stderr, "for esz = %llu, segmentation fault will follow.\n", esz);
    }
    /* due to realloc we have to reset ALL ev entries,
     * memory might have been moved */
    for (i = 1; i < esz; ++i) {
        ht->ev[i] = ht->ev[0] + (i*nv);
    }
    ...
    if (ht->hsz < (hl_t)pow(2,32)) {
        ...
        if (ht->hmap == NULL) {
            fprintf(stderr, "Enlarging hash table failed for hsz = %llu,\n", hsz);
            fprintf(stderr, "segmentation fault will follow.\n");
        }
    ...
    )
{
    ...
    if (size >= (ht->esz)) {
        ...
        if (ht->ev[0] == NULL) {
            fprintf(stderr, "Exponent storage needs too much memory on this machine,\n");
            fprintf(stderr, "reinitialization failed, esz = %llu\n", esz);
            fprintf(stderr, "segmentation fault will follow.\n");
        }
    ...
    )
{
    ...
    if (ht->ev == NULL) {
        fprintf(stderr, "Computation needs too much memory on this machine,\n");
        fprintf(stderr, "cannot reset ht->ev, esz = %llu\n", esz);
        fprintf(stderr, "segmentation fault will follow.\n");
    }
    exp_t *tmp  = (exp_t *)malloc(
    ...
    if (tmp == NULL) {
        fprintf(stderr, "Computation needs too much memory on this machine,\n");
        fprintf(stderr, "resetting table failed, esz = %llu\n", esz);
        fprintf(stderr, "segmentation fault will follow.\n");
    }
    for (k = 0; k < esz; ++k) {
        ht->ev[k]  = tmp + k*nv;
