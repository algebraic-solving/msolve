$(pwd)/msolve -f input_files/$file.ms -o test/diff/$file.3.res \
      --random-seed $seed \
      -g 2 -l 2 -t 1
if [ $? -gt 0 ]; then
    print_exit 3
fi

diff_gb_output test/diff/$file.3.res output_files/$file.g2.res
if [ $? -gt 0 ]; then
    print_exit 4
fi

rm test/diff/$file.3.res

$(pwd)/msolve -f input_files/$file.ms -o test/diff/$file.23.res \
      --random-seed $seed \
      -g 2 -l 2 -t 2
if [ $? -gt 0 ]; then
    print_exit 23
fi

diff_gb_output test/diff/$file.23.res output_files/$file.g2.res
if [ $? -gt 0 ]; then
    print_exit 24
fi

rm test/diff/$file.23.res

$(pwd)/msolve -f input_files/$file.ms -o test/diff/$file.43.res \
      --random-seed $seed \
      -g 2 -l 44 -t 1
if [ $? -gt 0 ]; then
    print_exit 43
fi

diff_gb_output test/diff/$file.43.res output_files/$file.g2.res
if [ $? -gt 0 ]; then
    print_exit 44
fi

rm test/diff/$file.43.res

$(pwd)/msolve -f input_files/$file.ms -o test/diff/$file.63.res \
      --random-seed $seed \
      -g 2 -l 44 -t 2
if [ $? -gt 0 ]; then
    print_exit 63
fi

diff_gb_output test/diff/$file.63.res output_files/$file.g2.res
if [ $? -gt 0 ]; then
    print_exit 64
fi

rm test/diff/$file.63.res
